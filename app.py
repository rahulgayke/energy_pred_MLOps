...
def return_prediction_dt(dt_1, dt_2, scaler_x, scaler_y1, scaler_y2, sample_json):
...
def return_prediction_rf(rf_1, rf_2, scaler_x, scaler_y1, scaler_y2, sample_json):
    ...
    Heating_Load = rf_1.predict(sample)
    Heating_Load = scaler_y1.inverse_transform(Heating_Load.reshape(-1, 1))
    
    Cooling_Load = rf_2.predict(sample)
    Cooling_Load = scaler_y2.inverse_transform(Cooling_Load.reshape(-1, 1))
    
    return Heating_Load, Cooling_Load

def return_prediction_svr(svr_1, svr_2, scaler_x, scaler_y1, scaler_y2, sample_json):
    # Fields for input json
    Relative_compactness = sample_json['Relative_compactness']
    Surface_area = sample_json['Surface_area']
    Wall_area = sample_json['Wall_area']
    ...
    Heating_Load = svr_1.predict(sample)
    Heating_Load = scaler_y1.inverse_transform(Heating_Load.reshape(-1, 1))
    
    Cooling_Load = svr_2.predict(sample)
    Cooling_Load = scaler_y2.inverse_transform(Cooling_Load.reshape(-1, 1))
...
dt_1 = joblib.load("./models/dt_1.joblib")
dt_2 = joblib.load("./models/dt_2.joblib")
rf_1 = joblib.load("./models/rf_1.joblib")
rf_2 = joblib.load("./models/rf_2.joblib")
svr_1 = joblib.load("./models/svr_1.joblib")
svr_2 = joblib.load("./models/svr_2.joblib")
...
def prediction():
    ...
    # results = return_prediction_dt(dt_1=dt_1, dt_2=dt_2, scaler_x=scaler_x, scaler_y1=scaler_y1, scaler_y2=scaler_y2, sample_json=content)
    results = return_prediction_rf(rf_1=rf_1, rf_2=rf_2, scaler_x=scaler_x, scaler_y1=scaler_y1, scaler_y2=scaler_y2, sample_json=content)
    # results = return_prediction_svr(svr_1=svr_1, svr_2=svr_2, scaler_x=scaler_x, scaler_y1=scaler_y1, scaler_y2=scaler_y2, sample_json=content)
